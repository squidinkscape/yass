...
def bhsrez(ticker,interval):
    ...
    try:
        ...
        if breakout:
            brk = 'BREAKOUT!'
        rezstring = f'''{ticker} --> Rs. {round(price,2)} \n \
...
def alertthread(window):
    ...
    while True:
        ...
        for ticker,val in WATCHMAP.items():
            ...
            plx.plot_size(75,35)
            plx.plot(x,y,label='close',marker='x')
            plx.scatter(x,y1,label='fast_dema',marker='.')
...
def scrapedailydf(choice='volume',yfinance=True):
    ...
    pool = mp.Pool(NCORES)
    matches = pool.map_async(scrapetickers, STONKURLS)
    ...
    if choice == 'gainers':
        ...
        df = matches.get()[1]["Symbol"].dropna().to_frame().head(250).drop_duplicates().reset_index(drop=True)
        pool.close()
        pool.join()

    if choice == 'losers':
        df = matches.get()[2]["Symbol"].dropna().to_frame().head(250).drop_duplicates().reset_index(drop=True)
        #df2 = matches.get()[1]["Symbol"].dropna().to_frame().head(150)
...
def chunkpickdf(choice='volume',interval='day'):
    ...
    try:
        ...
        cp(text2art('CALC',font='random'),c='purple on black')
    ...
    while True:
                ...
                if liststr == None or liststr == '':
                    ...
                    for tickr in liststr:
                        try:
                            ...
                            tickrs.append(tickr)
                            prices.append(float(df.iloc[-1,4]))
                        except:
                    ...
                    rupees = sg.popup_get_text('rupees in',title='amount in rs',
                                            no_titlebar=True)
                    if rupees in (None,'Cancel',''):
                        rupees = '1000.0'
                    window.perform_long_operation(lambda: optlistfolio(tickrs,prices,float(rupees)), '-listfolio-')
        ...
        if event == '-listfolio-' or event == '-folioread-':
            ...
            #print(tabulate(df,headers='keys',tablefmt='psql'))
                ...
                savecorr = sg.popup_ok('plot correlation?',title=SAVELOGO)

                if savecorr == 'Ok' or savecorr:
                    corr_df = f_df.pct_change().apply(lambda x: np.log(1+x)).corr()
                    corrfig = plotcorrdf(corr_df)
                    popupimage('corr',corrfig,'correlation matrix')
