...
def opt_folio_ratio(buy_df,rupees=10000.0,numbaz=3000, rf=0.01):
    ...
    for indx, symbol in enumerate(f_df.columns):
        ...
        try:
            ...
            pricerat = rupees/float(buy_df[buy_df['Symbol']==symbol]['Price'].iloc[0])
            data[symbol+'_quantity'] = [int(math.floor(w[indx]*pricerat)) for w in p_weights]
        
        except Exception as e:
            #do nothing
            print(f'price error {symbol}')
            print(e)
            continue
...
def optlistfolio(tickers,prices,rupees=10000,numbaz=3000,rf=0.01):
    df = pd.DataFrame()
    df['Symbol'] = pd.Series(tickers)
    df['Price'] = pd.Series(prices)
    f_df, assets, folios, figz = opt_folio_ratio(df,rupees,numbaz,rf)
    return df,f_df, assets, folios, figz

def plotcorrdf(df_corr):
    plt.rcParams["font.family"] = "OCR A Std"
    plt.rcParams["figure.autolayout"] = True
    plt.style.use('dark_background')
    ...
    plt.title(f'CORRELATION MATRIX',fontsize='xx-small')
    plt.tight_layout()
    plt.savefig(f"plots\\stonkin_correlations_{datetime.datetime.now().date()}.png")
    plt.close('all')
    return f"plots\\stonkin_correlations_{datetime.datetime.now().date()}.png"

if __name__ == "__main__":
    import time
    tickers = ['doge-usd','eth-usd','btc-usd','msft']
    prices = [10,1,20,30]
    start = time.time()
    a = optlistfolio(tickers,prices)
